...
<head>
  ...
  <title>FingerPass - Listar Turma</title>
...
<body>
  ...
  <div class="table-wrapper">
    <div class="table-header">
      <h2>Listar Turmas</h2>
      <a href="../turma/form_turma.php" class="btn-adicionar">Adicionar +</a>
    </div>
    ...
    <div class="table-container">
      <table class="tabela-7col">
        ...
        <tbody>
          ...
          $sql = "SELECT t.id_turma, t.n_turma, t.turno, t.contra_turno, c.nome AS curso_nome 
                  FROM turma t 
          ...
          if (mysqli_num_rows($res) > 0) {
            while ($linha = mysqli_fetch_assoc($res)) {
              ...
              echo '<td>' . ($linha['turno'] ? $linha['turno'] : '-') . '</td>';
              echo '<td>' . ($linha['contra_turno'] ? $linha['contra_turno'] : '-') . '</td>';
              echo '<td>' . ($linha['curso_nome'] ? $linha['curso_nome'] : '<em>Sem curso</em>') . '</td>';
              echo '<td>
                      <a href="editar_turma.php?id_turma=' . $linha['id_turma'] . '" class="btn-icon">
                        <img src="../../../img/EDT001.png" alt="Editar">
                      </a>
                    </td>';
              echo '<td>
                      <a href="excluir_turma.php?id_turma=' . $linha['id_turma'] . '" class="btn-icon excluir" onclick="return confirm(\'Tem certeza que deseja excluir esta turma?\')">
